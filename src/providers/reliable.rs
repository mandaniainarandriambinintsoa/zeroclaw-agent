        ...
        for current_model in &models {
            for (provider_name, provider) in &self.providers {
                        ...
                        Ok(resp) => {
                            if attempt > 0 || *current_model != model {
                                tracing::info!(
                                    ...
                                    attempt,
                                    original_model = model,
                                    "Provider chat recovered (failover/retry)"
                                );
                            }
                            return Ok(resp);
                        }
                        Err(e) => {
                            ...
                            if attempt < self.max_retries {
                                ...
                                tracing::warn!(
                                    ...
                                    reason = failure_reason,
                                    error = %error_detail,
                                    "Provider chat call failed, retrying"
                                );
                                tokio::time::sleep(Duration::from_millis(wait)).await;
                                backoff_ms = (backoff_ms.saturating_mul(2)).min(10_000);
                            }
                ...
                tracing::warn!(
                    provider = provider_name,
                    model = *current_model,
                    "Exhausted retries for chat, trying next provider/model"
                );
            }
        }

        anyhow::bail!(
            "All providers/models failed for chat. Attempts:\n{}",
            failures.join("\n")
        )
    }
